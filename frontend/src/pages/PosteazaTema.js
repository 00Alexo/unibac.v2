...
export default function PosteazaTema() {
  ...
  const handleSubmit = async () => {
    const response = await fetch(`${process.env.REACT_APP_API}/api/class/posteazaTema`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({username: user.username, classId, content, titlu, deadline})
    })
    ...
    if(response.ok){
        navigate(`/clase/${classId}/teme/${json.id}`);
    }
  }
